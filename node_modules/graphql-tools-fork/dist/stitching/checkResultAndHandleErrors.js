...
function handleList(type, list, errors, subschema, context, info, skipTypeMerging) {
    ...
    list = list.map(function (listMember, index) { return handleListMember(graphql_1.getNullableType(type.ofType), listMember, index, childErrors[index] || [], subschema, context, info, skipTypeMerging); });
    return list;
}
...
function handleObject(type, object, errors, subschema, context, info, skipTypeMerging) {
    ...
    var targetSubschemas = mergedTypeInfo && mergedTypeInfo.subschemas;
    if (!targetSubschemas) {
        return object;
    }
...
function collectSubFields(info, typeName) {
    ...
    info.fieldNodes.forEach(function (fieldNode) {
        subFieldNodes = execute_1.collectFields({ schema: info.schema, variableValues: info.variableValues, fragments: info.fragments }, info.schema.getType(typeName), fieldNode.selectionSet, subFieldNodes, visitedFragmentNames);
    });
